...
export default function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowSuccess(true);  // SHOW POPUP ON SUCCESS
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <div className="bg-white p-8 rounded-2xl shadow-lg w-96">
          ...
          />
          <button onClick={handleLogin} className="w-full bg-black text-white p-3 rounded-xl hover:opacity-90 transition">
            Login
          </button>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto bg-white p-8 rounded-2xl shadow-lg">
        ...
        <form onSubmit={handleSubmit}>
          ...
          {expenses.map((expense, index) => (
            <div key={index} className="mb-8 p-6 border rounded-2xl bg-gray-50">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-semibold">Expense #{index + 1}</h2>
                {expenses.length > 1 && (
                  <button type="button" onClick={() => removeExpense(index)} className="text-red-500 text-sm">
                    Remove
                  </button>
                )}
              </div>
              <div className="grid md:grid-cols-2 gap-4">
                <input type="date" required className="p-3 border rounded-xl" value={expense.date} onChange={(e) => handleExpenseChange(index, "date", e.target.value)} />
                <select required className="p-3 border rounded-xl" value={expense.type} onChange={(e) => handleExpenseChange(index, "type", e.target.value)}>
                ...
                <input type="text" placeholder="Vendor Name" required className="p-3 border rounded-xl" value={expense.vendor} onChange={(e) => handleExpenseChange(index, "vendor", e.target.value)} />
                <input type="number" step="0.01" placeholder="Total Amount" required className="p-3 border rounded-xl" value={expense.total} onChange={(e) => handleExpenseChange(index, "total", e.target.value)} />
                <input type="number" step="0.01" placeholder="Tax" required className="p-3 border rounded-xl" value={expense.tax} onChange={(e) => handleExpenseChange(index, "tax", e.target.value)} />
                <input type="file" multiple className="p-3 border rounded-xl" onChange={(e) => handleFileChange(index, e.target.files)} />
              </div>
            </div>
          ))}

          <button type="button" onClick={addExpense} className="mb-6 px-5 py-3 border rounded-xl hover:bg-gray-100 transition">
            + Add Another Expense
          </button>
          <div>
            <button type="submit" className="w-full bg-black text-white p-4 rounded-2xl text-lg hover:opacity-90 transition">
              Submit Expenses
            </button>
          </div>
        </form>
